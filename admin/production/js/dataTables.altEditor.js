...
var initValidation = function(){
  ...
  $('form[name="altEditor-form"] #phone2').filter(':text').each(function( i ){
    ...
    $('form[name="altEditor-form"] #district_name').filter(':text').each(function( i ){
        var errorLabel = "#"+ $(this).attr("id") + "label";
        if ($.trim($(this).val()) === "") 
        {
    ...
    $('form[name="altEditor-form"] #zone_name').filter(':text').each(function( i ){
        var errorLabel = "#"+ $(this).attr("id") + "label";
        if ($.trim($(this).val()) === "") 
        {
     ...
     $('form[name="altEditor-form"] #employee_name').filter(':text').each(function( i ){
        var errorLabel = "#"+ $(this).attr("id") + "label";
        if ($.trim($(this).val()) === "") 
        {
    ...
    $('form[name="altEditor-form"] #school_name').filter(':text').each(function( i ){
        var errorLabel = "#"+ $(this).attr("id") + "label";
        if ($.trim($(this).val()) === "") 
        {
  ...
  if (checkUrl == ajaxUrl && count == 0) 
  {
      count++;
        ...
        if(act == 'deleteRow'){
            ...
            else if(page == 'viewOrganisation'){   
                dt.ajax.reload();
            } 
            else if(page == 'viewParticipants'){   
                dt.ajax.reload();
            } 
        }
        else if(act == 'editRow'){
        ...
        $('#altEditor-modal').modal('hide');
         ...
         if (act == 'deleteRow') //to delete data from data tables
        {
         // when delete is successfull
            ...
            if (ajaxUrl == 'galleryDAO.php')
            {
              dt.ajax.reload();
            }
            if (ajaxUrl == 'orderDAO.php')
            {
              dt.ajax.reload();
            }
            if (ajaxUrl == 'districtDAO.php')
            {
              dt.ajax.reload();
            }
            if (ajaxUrl == 'zoneDAO.php')
            {
              dt.ajax.reload();
            }
            if (ajaxUrl == 'schoolDAO.php')
            {
              dt.ajax.reload();
            }
            if (ajaxUrl == 'organisationDAO.php')
            {
              dt.ajax.reload();
            }
            if (ajaxUrl == 'participantDAO.php')
            {
              dt.ajax.reload();
            }
        }else if (act == 'editRow') //to edit data from data tables
        {
         // when edit is successfull
            new PNotify({
            title: 'Status',
